...
public class GetBuildingUseCase {
    ...
    private static final String TAG = GetBuildingUseCase.class.getSimpleName();
        ...
        SitumSdk.communicationManager().fetchBuildings(new Handler<Collection<Building>>(){
            ...
            public void onSuccess(Collection<Building> buildings) {
                if(!buildings.isEmpty()){
                    ...
                    //Get all the floors of the building
                    SitumSdk.communicationManager().fetchFloorsFromBuilding(currBuilding.getIdentifier(), new Handler<Collection<Floor>>() {
                        ...
                        public void onSuccess(Collection<Floor> floors) {
                            if(!floors.isEmpty()){
                                currFloor = floors.iterator().next(); //get the first floor

                                //Get floor plan of first floor
                                SitumSdk.communicationManager().fetchMapFromFloor(currFloor, new Handler<Bitmap>() {
                                    @Override
                                    public void onSuccess(Bitmap bitmap) {
                                        if(hasCallback()){
                                            callback.onSuccess(currBuilding, currFloor, bitmap); //set data into onSuccess
                                        }
                                        clearCallback();
                                    }
